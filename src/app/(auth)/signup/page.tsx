...
// Define Zod schema for the sign-up form
...
// Inline SVG for Google icon (reuse from SignIn)
const GoogleIcon = () => (
...
export default function SignUpPage() {
  ...
  const { toast } = useToast();
   const [isSubmitting, setIsSubmitting] = React.useState(false); // Local loading state
  ...
  const onSubmit = async (data: SignUpFormData) => {
     ...
     if (user) {
      ...
      // Error handling is done within signUp via toast
  ...
  return (
    <Card className="mx-auto max-w-sm">
      ...
      <CardFooter className="text-center text-sm">
        لديك حساب بالفعل؟{' '}
        <Link href="/signin" className="text-primary hover:underline mr-1">
          تسجيل الدخول
        </Link>
      </CardFooter>
    </Card>
